  ...
  .then(data => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${key}&units=metric`)
      ...
      .then(data => {
        ...
        const humidity = main.humidity; // get the humidity data from the main object
        const weatherParagraph = document.getElementById('weather');
        weatherParagraph.textContent = `${temperature}°C ${description.charAt(0).toUpperCase() + description.slice(1)}, ${humidity}% Humidity`; // add humidity to the text content
